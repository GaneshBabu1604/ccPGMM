...
#Generating dataset with mildly overlapping clusters
sim_data = 1
...
#Generating the mildly overlapping data from four mildly overlapping Factor Analyzers(One for each cereal type and background)
for(g in 1:G){
...
for(d in c(10,20)){
  ...
  for(M in c(10,25,50,100)){
    for (constrained in c(0,1)){
      ...
      n_result = 1
      for(s in result){
        location = paste('mildlyoverlappingcluster_Data_',as.character(sim_data),'_M',as.character(M),'_d',as.character(d),'_cons',as.character(constrained),'_',as.character(n_result),'.RData',sep = '')
        save(s,file = location)
        n_result=n_result+1
      }
      location = paste('mildlyoverlappingcluster_Data_',as.character(sim_data),'_M',as.character(M),'_d',as.character(d),'_cons',as.character(constrained),'_creation_time.RData',sep = '')
...
for(d in c(10,20)){
  for(M in c(10,25,50,100)){
    for(c in c(0,1)){
      for(e in c(0,1)){
        print(c(c,e))
        loc = paste('mildlyoverlappingcluster_Data_',as.character(sim_data),sep ='')
...
full_dbscan <- dbscan(as.matrix(temp_simulated_data), minPts = 202, eps=20)
