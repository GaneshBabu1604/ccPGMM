...
for(d in c(10,20)){
  ...
  for(r in c(10,25,50,100)){
    for (constrained in c(0,1)){
      ...
      n_result = 1
      for(s in result){
        location = paste('wellseparatedcluster_Data_',as.character(sim_data),'_r',as.character(r),'_d',as.character(d),'_cons',as.character(constrained),'_',as.character(n_result),'.RData',sep = '')
        save(s,file = location)
        n_result=n_result+1
      }
      location = paste('wellseparatedcluster_Data_',as.character(sim_data),'_r',as.character(r),'_d',as.character(d),'_cons',as.character(constrained),'_creation_time.RData',sep = '')
...
abline(h=11, lt=2, col="blue")
full_dbscan <- dbscan(as.matrix(temp_simulated_data), minPts = 202, eps=11)
adjustedRandIndex(cluster_labels,full_dbscan$cluster)
